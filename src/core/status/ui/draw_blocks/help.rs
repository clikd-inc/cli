...
impl HelpInfo {
    ...
    /// Generate the `clikd` name span + metadata
    ...
    fn custom_text<'a>(colors: AppColors, _keymap: &Keymap, zone: Option<&TimeZone>) -> Line<'a> {
        ...
        let zone_name = zone.and_then(|i| i.iana_name()).unwrap_or("Etc/UTC");
        let zone = zone_name.strip_prefix("Etc/").unwrap_or(zone_name);
