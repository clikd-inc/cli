...
impl AppData {
    ...
    /// Check if the selected container is a dockerised version of clikd
    /// So that can disallow commands to be send
    /// Is a shabby way of implementing this
    pub fn is_clikd(&self) -> bool {
        self.get_selected_container().is_some_and(|i| i.is_clikd)
    }

    /// Check if selected container is clikd and also that clikd is being run in a container
    pub fn is_clikd_in_container(&self) -> bool {
        self.get_selected_container()
            .is_some_and(|i| i.is_clikd && self.config.in_container)
    }
            ...
            if let Some(id) = i.id.as_ref() {
                let name = i.names.as_mut().map_or(String::new(), |names| {
                    names.first_mut().map_or(String::new(), |f| {
                        ...
                        let full_name = (*f).to_string();

                        if full_name.starts_with("clikd_") {
                ...
                if let Some(item) = self.get_any_container_by_id(&id) {
                    ...
                    let container = ContainerItem::new(
                        created, id, image, is_clikd, name, ports, state, status,
                    );
...
mod tests {
    ...
    use super::*;
    use crate::core::status::tests::{gen_appdata, gen_container_summary, gen_containers};
    ...
    fn test_app_data_update_log_by_id() {
        ...
        assert_eq!(result.as_ref().expect("BUG: test case should succeed").selected(), Some(2));
    ...
    fn test_app_data_logs_start() {
        ...
        let result = app_data.get_log_state();
        assert!(result.is_some());
        assert_eq!(result.as_ref().expect("BUG: test case should succeed").selected(), Some(0));
        assert_eq!(result.expect("BUG: test case should succeed").offset(), 0);
    ...
    fn test_app_data_logs_next() {
        ...
        let result = app_data.get_log_state();
        assert!(result.is_some());
        assert_eq!(result.as_ref().expect("BUG: test case should succeed").selected(), Some(1));
        assert_eq!(result.expect("BUG: test case should succeed").offset(), 0);
    ...
    #[test]
    /// Update logs don't work if container is_clikd: true
    fn test_app_data_update_log_by_id_is_clikd() {
        let (ids, mut containers) = gen_containers();
        containers[0].is_clikd = true;
        let mut app_data = gen_appdata(&containers);
        let logs = (1..=3).map(|i| format!("{i} {i}")).collect::<Vec<_>>();
