...
impl Ui {
    ...
    /// Draw either the Error, or main clikd ui, to the terminal
...
fn draw_frame(
    ...
    if let Some(id) = fd.delete_confirm.as_ref() {
        app_data.lock().get_container_name_by_id(id).map_or_else(
            || {
                // If a container is deleted outside of clikd but whilst the Delete Confirm dialog is open, it can get caught in kind of a dead lock situation
