...
impl ExecMode {
    ...
    /// If the container is clikd, it will always return None
    pub async fn new(app_data: &Arc<Mutex<AppData>>, docker: &Arc<Docker>) -> Option<Self> {
        let is_clikd = app_data.lock().is_clikd();
        if is_clikd {
            return None;
        }
    ...
    /// exec into the container using the external docker cli, the result it just piped into clikd
